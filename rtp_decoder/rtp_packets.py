...
FILES = [{'rate': 6, 'file': 'neo_6kbs_2g_20121220_1153.pcap'},
...
def extract_rtp_from_pcap(filename):
    ...
    """ Decode RTP frames from a pcap file
        RTP is used by webrtc for the audio data in Neo watch
    """
    rtp_frames = []
...
def generate_rtp_packet(packet_vars):
    """ Generate an RTP packet with the given dat
        Example Usage:
...
def main():
    """ Main Program
        ...
        Extract the RTC data rate with headers removed from the
        given pcap files
    """
    for file in FILES:
